...
async function containerCreate() {
  ...
  await buildProject(gitHubAccount.access_token, orgName, repoName, branch, containerUid, dockerfileTemplateName, preSetupSteps, repoConfig.machine.setup);
...
function buildProject(gitHubToken, orgName, repoName, branch, containerUid, templateName, preSetupSteps, machineSetup = ':' /* : is bash noOp */) {
  return new Promise((resolve, reject) => {
    const path = require('path');
    const command = [
      'bash',
    ...
    try {
      ...
      try {
        ...
        if (runErr.message && runErr.message.includes('port is already allocated')) {
          ...
          try {
            recursiveRunResults = runProject(repoConfig, containerUid);
          } catch (err) {
            return reject(err);
          }
