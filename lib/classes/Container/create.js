...
function containerCreate(callback) {
  ...
  waterfallSteps.push((watchedRepo, repoConfig, gitHubToken, containerRowId, cb) => {
    ...
    const templatesNeeded = languageNames.reduce((templates, languageName) => {
      templates.push(`/${languageName}/${languages[languageName].version}`);
      return templates;
    }, ['base']);

    function buildTemplatePart(currentTemplateSubname) {
      const current = templatesNeeded.shift();

      newTemplateSubname = arguments.length === 0 ? 'base' :
        newTemplateSubname === 'base' ? current.split('/').pop() :
        `${newTemplateSubname}__________${current.split('/').pop()}`; // _s used to signify a chain of languages

      const templateName = `conjure:${newTemplateSubname}`;

      if (current === undefined) {
        return cb(null, watchedRepo, repoConfig, gitHubToken, containerRowId, templateName);
      }

      const path = require('path');
      const command = `docker build -t "${templateName}" `;
    }
    buildTemplatePart()
  });
  ...
  waterfallSteps.push((watchedRepo, repoConfig, gitHubToken, containerRowId, templateName, cb) => {
    ...
    const path = require('path');
    const command = `bash ./build/project.sh "${templateName}" "https://${gitHubToken}:x-oauth-basic@github.com/${orgName}/${repoName}.git" "${branch}" "${containerUid}" "${preSetupSteps}" "${repoConfig.machine.setup || bashNoOp}"`;
