...
function spinUpProject(watchedRepo, orgName, repoName, repoConfig, containerUid) {
  return new Promise(async (resolve, reject) => {
    ...
    let taskRegistered = ecsRetrieveTaskDefinition(watchedRepo);
    if (!taskRegistered) {
      taskRegistered = await ecsRegisterTaskDefinition(watchedRepo, repoConfig);
    }

    const cluster = await ecsCreateCluster(watchedRepo);
    const task = await ecsRunTask(watchedRepo);
  });
}
  ...
  return new Promise((resolve, reject) => {
    ...
    ecs.listTaskDefinitions(familyPrefix, (err, data) => {
      ...
      if (Array.isArray(data.taskDefinitionArns) && data.taskDefinitionArns.length) {
        return resolve(taskDefinitionArns[ taskDefinitionArns.length - 1]);
      }
...
function ecsRunTask(repoConfig) {
  return new Promise((resolve, reject) => {
    const taskParam = {
      cluster: `watched-${watchedRepo.id}`,
      taskDefinition: `conjure/watched-${watchedRepo.id}:1`
    };
