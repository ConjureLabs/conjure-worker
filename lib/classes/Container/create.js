...
async function containerCreate(callback) {
  ...
  waterfallSteps.push(cb => {
    let watchedRepo;
    try {
      watchedRepo = this.payload.getWatchedRepoRecord();
    } catch (err) {
      return cb(err);
    }
  ...
  waterfallSteps.push(async (watchedRepo, cb, asyncBreak) => {
    const DatabaseTable = require('conjure-core/classes/DatabaseTable');

    let records;
    try {
      records = await DatabaseTable.select('container', {
        repo: watchedRepo.id,
        branch: branch,
        is_active: true
      });
  ...
  // get github client
  waterfallSteps.push(async (watchedRepo, cb) => {
    let gitHubAccount;
    try {
      gitHubAccount = await this.payload.getGitHubAccount();
    } catch (err) {
      return cb(err);
    }
  ...
  waterfallSteps.push(async (watchedRepo, repoConfig, gitHubToken, cb) => {
    const DatabaseTable = require('conjure-core/classes/DatabaseTable');
    let rows;
    try {
      rows = await DatabaseTable.insert('container', {
        repo: watchedRepo.id,
        branch: branch,
  ...
  waterfallSteps.push(async (watchedRepo, containerRowId, hostPort, containerId, cb) => {
    const DatabaseTable = require('conjure-core/classes/DatabaseTable');
