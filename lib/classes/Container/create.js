...
function spinUpProject(watchedRepo, repoConfig) {
  return new Promise(async (resolve, reject) => {
    ...
    log.info('project should be running via Fargate');
  });
}
...
function ecsRetrieveTaskDefinition(watchedRepo) {
  return new Promise((resolve, reject) => {
    const taskListParam = {
      familyPrefix: `watched-${watchedRepo.id}`
    };
...
function ecsRunTask(watchedRepo, taskDefinitionRevision) {
  return new Promise((resolve, reject) => {
    const taskParam = {
      ...
      taskDefinition: `watched-${watchedRepo.id}:${taskDefinitionRevision}`,
      launchType: 'FARGATE',
      count: 1,
