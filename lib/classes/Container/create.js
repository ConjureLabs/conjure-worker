  ...
  waterfallSteps.push((watchedRepo, repoConfig, gitHubToken, containerRowId, cb) => {
    ...
    function buildTemplatePart(lastTemplateSubname) {
      ...
      const newTemplateSubname = arguments.length === 0 ? 'base' :
        ...
        `${lastTemplateSubname}__________${current.split('/').pop()}`; // _s used to signify a chain of languages
      ...
      const command = `bash ./build/dockerfile-template.sh "${current}.Dockerfile" "${templateName}" "${fromTemplate}"`;
