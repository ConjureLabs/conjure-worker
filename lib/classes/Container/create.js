...
async function containerCreate() {
  ...
  await buildProject(gitHubAccount.access_token, containerRowId, orgName, repoName, branch, containerUid, dockerfileTemplateName, preSetupSteps, repoConfig.machine.setup);
...
function buildProject(gitHubToken, containerRowId, orgName, repoName, branch, containerUid, templateName, preSetupSteps, machineSetup = ':' /* : is bash noOp */) {
  return new Promise((resolve, reject) => {
    const path = require('path');
    const command = [
      'bash',
      ...
      `${branch}`,
      `${config.aws.account.id}.dkr.ecr.${config.aws.default.region}.amazonaws.com/conjure/watched-${containerRowId}`,
      // `${containerUid}`,
      `${preSetupSteps}`,
      `${machineSetup}`
    ];
