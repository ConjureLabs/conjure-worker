...
async function containerCreate() {
  ...
  await buildProject(gitHubAccount.access_token, 1, orgName, repoName, branch, containerUid, dockerfileTemplateName, preSetupSteps, repoConfig.machine.setup);
  await spinUpProject(watchedRepo, orgName, repoName, repoConfig, containerUid);
...
function buildProject(gitHubToken, watchedRepoId, orgName, repoName, branch, containerUid, templateName, preSetupSteps, machineSetup = ':' /* : is bash noOp */) {
  return new Promise((resolve, reject) => {
    const path = require('path');
    const command = [
      'bash',
...
function spinUpProject(watchedRepo, orgName, repoName, repoConfig, containerUid) {
  return new Promise(async (resolve, reject) => {
    const ecrRepoRecord = await getEcrRepoRecord(watchedRepo, orgName, repoName);
  });
}

function getEcrRepoRecord(watchedRepo, orgName, repoName) {
  return new Promise(async (resolve, reject) => {
    ...
    if (ecrRepoRecords.length > 0) {
      ...
      */
      const createRepoCmd = `aws ecr create-repository --repository-name ${orgName}/${repoName}`;
      let createRepoStdout;

      try {
        createRepoStdout = await exec(createRepoCmd);
      } catch(createRepoErr) {
        return reject(createRepoErr);
      }

      try {
        createRepoStdout = JSON.parse(createRepoStdout);

        // ensuring that the dot notation exists
        if (!createRepoStdout.repository.repositoryName) {
          throw new ContentError('Missing content while creating repo record');
        }
      } catch(createRepoErr) {
        return reject(createRepoErr);
      }
      ...
      ecrRepoRecord = await DatabaseTable.insert('ecr_repo', {
        watched_repo_id: watchedRepo.id,
        name: createRepoStdout.repository.repositoryName,
        added: new Date()
      });
    }
