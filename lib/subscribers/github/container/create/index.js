...
const queue = new Queue('exchange', 'containers', 'container.create');
...
queue.subscribe((err, message) => {
  ...
  log.info(`creating github container (${message.payload.orgName}/${message.payload.repoName} --> ${message.payload.branch})`);
  ...
  container.create(err => {
    ...
    log.info(`created github container (${message.payload.orgName}/${message.payload.repoName} --> ${message.payload.branch})`);
    message.ack();
  });
});
