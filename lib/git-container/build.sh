...
echo "$DOCKERFILE_CONTENT" > "$TEMP_DOCKERFILE_DIR/$CONTAINER_NAME.Dockerfile";

docker build -t "$CONTAINER_NAME:latest" -f "$TEMP_DOCKERFILE_DIR/$CONTAINER_NAME.Dockerfile" .;
